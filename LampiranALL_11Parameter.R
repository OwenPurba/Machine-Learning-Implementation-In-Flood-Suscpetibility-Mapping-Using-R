...
# Matriks koefisien korelasi parameter terpilih
datakor1 <- datakor[,c(-4,-12,-14:-23)] #hapus
corr <-round(cor(datakor1), 3)
head(corr[, 1:6])
fontsize <- 15
...
plot
#VIF
Model2 <- lm(training_df$label~training_df$ELEVASI+training_df$SLOPE+training_df$CH+training_df$JTS+training_df$TL+training_df$LD+training_df$MANNNING+training_df$TWI+training_df$JT+training_df$NDBI+training_df$B5)
summary(Model2)
vif(Model2)
vif_values2 <- vif(Model2)
#paruh1
vif_data <- data.frame(variable = names(vif_values2), vif = vif_values2)
ggplot(vif_data, aes(x = variable, y = vif)) +
  geom_bar(stat = "identity", fill = "Darkblue", width = 0.5) +
  geom_text(aes(label = round(vif, 2)), vjust = -0.5, size = 4, color = "black") +
  labs(title = "Variance Inflation Factor (VIF) Plot",
       x = "Variables",
       y = "VIF Value") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size= 10),
        plot.title = element_text(hjust = 0.5)) +
  scale_x_discrete(labels = c("B5","Curah Hujan", "Ëlevasi", "Jenis Tanah", "Jarak Terhadap Sungai", "Kerapatan Sungai", "Manning's RC", "NDBI", "SLOPE", "Tutupan Lahan", "TWI"))
